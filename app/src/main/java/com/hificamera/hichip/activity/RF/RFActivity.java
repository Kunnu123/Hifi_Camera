...
import com.hificamera.thecamhi.activity.ScanQRCodeActivity;
...
public class RFActivity extends HiActivity implements ICameraIOSessionCallback, OnCheckedChangeListener {
	...
	private void setListeners() {
		mLvListRf.setOnMenuItemClickListener(new SwipeMenuListView.OnMenuItemClickListener() {
			@Override
		...
		mLvListRf.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (!switch_bf.isChecked()) {
					HiToast.showToast(RFActivity.this, "Please turn on RF arming first!");
					return;
				}
				if (mIsHaveKey) {
					if (position == 0) {
	...
	private void showClosePup() {
		...
		dialog.withTitle("确认关闭?").withMessage("Confirm to close?").withButton1Text("Confirm").withButton2Text("Don't close").setButton1Click(new OnClickListener() {
			@Override
	...
	private void initView() {
		...
		titleView.setTitle("Manage sensors\n" + mMyCamera.getUid());
				...
				switch (which) {
					...
					case TitleView.NAVIGATION_BUTTON_RIGHT:
						if (switch_bf.isChecked()) {
							...
							HiToast.showToast(RFActivity.this, "Please turn on the RF arming first");
						}
						break;
	...
	private class RfListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (mIsHaveKey && position == 0) {
				holder.tvName.setText("Remote control sensor");
