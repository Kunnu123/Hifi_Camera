...
public class EditCameraActivity extends HiActivity {
	...
	private void chickDone() {
		String str_nike = edt_nikename.getText().toString().trim();
		...
		if(isChangeName){
			if(str_nike.length()>31){
				str_nike=str_nike.substring(0,31);
			}
			if(!TextUtils.isEmpty(str_nike.trim())&&mCamera.getPushState()>0){
				UpnameUtils.upName(str_nike,this,mCamera);
			}
		}
