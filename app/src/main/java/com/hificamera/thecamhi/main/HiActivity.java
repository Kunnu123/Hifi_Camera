...
public class HiActivity extends FragmentActivity {
	...
	public void showjuHuaDialog() {
		if (mJhLoading == null) {
			mJhLoading = DialogUtils.createLoadingDialog(this, true,true);
		}
		mJhLoading.show();
	}
	public void showjuHuaDialog(boolean cancel) {
		if (mJhLoading == null) {
			mJhLoading = DialogUtils.createLoadingDialog(this,cancel, cancel);
		}
		mJhLoading.show();
	}
	public void  jumpToAgreement(){
		Intent intent = new Intent(HiActivity.this, WebViewActivity.class);
		intent.putExtra("title", getResources().getString(R.string.about_user_agreement));
		if (SystemUtils.isZh(HiActivity.this)) {
